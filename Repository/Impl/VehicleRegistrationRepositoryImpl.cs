    ...
    public class VehicleRegistrationRepositoryImpl:IVehicleRegistrationRepository
    {
        ...
        public async Task<IEnumerable<Vehicle>> FindAllVehicles()
        {
            return await _context.Vehicles.Where(r => r.IsDeleted == false)
                ...
                .Include(r=>r.AttachedOffice).Include(r=>r.VehicleType)
                .Include(r=>r.SupplierService).Include(r=>r.AttachedBranch)
                                      .ToListAsync();
        }

        public async Task<Vehicle> FindVehicleById(long Id)
        {
            return await _context.Vehicles.Include(r => r.CreatedBy)
                .Include(r => r.AttachedOffice).Include(r => r.VehicleType)
                .Include(r => r.SupplierService).Include(r => r.AttachedBranch).FirstOrDefaultAsync(v => v.Id == Id && v.IsDeleted == false);
        }
